...
export const AnalyticsProvider: React.FC<AnalyticsProviderProps> = ({ children }) => {
  ...
  // Check if analytics is enabled from localStorage
  const [isAnalyticsEnabled, setIsAnalyticsEnabled] = useState<boolean>(() => {
    const userSettings = localStorage.getItem('userSettings');
    if (userSettings) {
      try {
        const settings = JSON.parse(userSettings);
        return settings.privacy?.analyticsEnabled ?? true;
      } catch {
        return true;
      }
    }
  ...
  }, [currentPageView, interactionBuffer, adaptiveSampler, isAnalyticsEnabled]);
